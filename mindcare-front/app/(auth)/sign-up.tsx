...
const SignUp = () => {
  ...
  return (
    <ScrollView className="flex-1 bg-gradient-to-b from-blue-400 to-purple-600">
      <View className="flex-1">
        <ImageBackground source={require("../../assets/images/bg1.jpg")} className="w-full h-[250px] flex justify-end px-5" resizeMode="cover">
          <View className="relative w-full h-[250px] flex justify-end ">
            <Text className="text-4xl text-green-900 font-JakartaSemiBold mb-5">Create Your Account</Text>
          </View>
        </ImageBackground>
        ...
        <View className="p-5 bg-white rounded-t-3xl shadow-lg">
          <InputField label="Name" placeholder="Enter name" value={form.name} onChangeText={(value) => setForm({ ...form, name: value })} />
          <InputField label="Email" placeholder="Enter email" textContentType="emailAddress" value={form.email} onChangeText={(value) => setForm({ ...form, email: value })} />
          <InputField label="Password" placeholder="Enter password" secureTextEntry={true} textContentType="password" value={form.password} onChangeText={(value) => setForm({ ...form, password: value })} />
          <CustomButton title="Sign Up" onPress={onSignUpPress} className="mt-6 bg-green-700" />
          <OAuth />
          <Link href="/sign-in" className="text-lg text-center text-gray-300 mt-10">
            Already have an account? <Text className="text-green-600">Log In</Text>
        ...
        <ReactNativeModal isVisible={showSuccessModal}>
          <View className="bg-white px-7 py-9 rounded-2xl min-h-[300px]">
            <Text className="text-3xl font-JakartaBold text-center">Verified</Text>
            <Text className="text-base text-gray-400 font-Jakarta text-center mt-2">You have successfully created your account.</Text>
            <CustomButton
              title="Browse Home"
              onPress={() => {
